...
class SettingsPanel {
    ...
    createNavigation() {
        ...
        this.tabConfig.forEach((tab, index) => {
            ...
            tabButton.setAttribute('aria-selected', tab.id === this.activeTab ? 'true' : 'false');
            tabButton.setAttribute('tabindex', tab.id === this.activeTab ? '0' : '-1');
    ...
    handleTabKeydown(e, currentIndex) {
        ...
        switch (e.key) {
            case 'ArrowLeft':
            case 'ArrowUp':
                targetIndex = currentIndex > 0 ? currentIndex - 1 : this.tabConfig.length - 1;
                e.preventDefault();
                break;
                
            case 'ArrowRight':
            case 'ArrowDown':
                targetIndex = currentIndex < this.tabConfig.length - 1 ? currentIndex + 1 : 0;
                e.preventDefault();
                break;
